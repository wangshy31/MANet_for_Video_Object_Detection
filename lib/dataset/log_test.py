   ...
   # load annotations from cache
    ...
    for motion_range_id, motion_range in enumerate(motion_ranges):
        for area_range_id, area_range in enumerate(area_ranges):
            ...
            print 'gt_precent!!!!', gt_precent[0][0]
...
def main():
    multifiles=False
    pre = '/mnt/lustre/wangshiyao/workspace/VID/mxproject/models/flow_baseline/test/'
    detpath = pre+'resnet_v1_101_flownet_imagenet_vid_rfcn_end2end_ohem/VID_val_videos/results/det_VID_val_videos_all.txt'
    annopath = pre+'data/ILSVRC2015/Annotations/{0!s}.xml'
    vid_eval_motion(multifiles, detpath, annopath, imageset_file, classname_map, annocache, motion_iou_file, motion_ranges, area_ranges, ovthresh=0.5):
